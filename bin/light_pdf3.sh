...
IFS=$'\#'
for f in $@; do  pdftoppm -jpeg "$f" /tmp/1 ; convert -morphology Convolve DoG:15,100,0 -modulate 100,150,100 -negate -normalize -blur 0x0.7 -channel RBG -level 30%,100%,0.4 /tmp/1*.jpg "$f".pdf ; rm /tmp/1*.jpg ; touch -r "$f" "$f".pdf ;  mv "$f".pdf "$f" ; done
