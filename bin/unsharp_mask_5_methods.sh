...
if [ "$OPTION1" = "Просто чётче"  ] ; then OPTION=light-picture0.sh;
	else  OPTION=light-picture1.sh; fi
if [ "$OPTION1" = "Чётче, ещё толще линии"   ] ; then OPTION=light-picture2.sh; fi
if [ "$OPTION1" = "Для слепых сканов"   ] ; then OPTION=light-picture3.sh; fi
if [ "$OPTION1" = "БЕЛЫЙ ФОН"   ] ; then OPTION=white_background.sh; fi
